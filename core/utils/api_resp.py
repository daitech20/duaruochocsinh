...
def make_api_response(
    success: int = 1, data: Any = None, http_status: int = 200, **kwargs
):
    payloads = {"success": 1, "data": data}
    return Response(status=http_status, data=payloads)


def success_api_resp(data: Any, success: int = 1, **kwargs):
...
def custom_exception_handler(exc, context):
    try:
        print(exc)
        response_data = {"success": 0, "error": exc.detail}
        response = Response(response_data, status=exc.status_code)
    except AttributeError as e:
        response_data = {
            "success": 0,
            "error": "invalid data or exception"
        }
